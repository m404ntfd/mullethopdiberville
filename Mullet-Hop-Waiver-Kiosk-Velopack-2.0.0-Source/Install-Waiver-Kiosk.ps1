...
Write-Host ''
Write-Host 'Mullet Hop Waiver Kiosk Installer' -ForegroundColor Cyan
...
foreach ($folder in $searchFolders) {
    ...
    $setup = Get-ChildItem -LiteralPath $folder -Filter '*-Setup.exe' -File |
        Where-Object { $_.Name -like 'MulletHop.WaiverKiosk*' } |
...
if (-not $setup) {
    throw 'The Velopack Setup file is missing. Download it from https://github.com/m404ntfd/mullethopdiberville/releases/latest, place it beside this script, and try again.'
}
...
$installedExe = Join-Path $env:LOCALAPPDATA 'MulletHop.WaiverKiosk\current\MulletHopWaiverKiosk.exe'
if (-not (Test-Path -LiteralPath $installedExe)) {
    throw 'Installation completed, but the installed kiosk application could not be found.'
}

$startupShortcutPath = Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\Startup\Mullet Hop Waiver Kiosk.lnk'
$previousStartupPreference = Test-Path -LiteralPath $startupShortcutPath
if ($previousStartupPreference) {
    $startupAnswer = 'Y'
}
else {
    $startupAnswer = Read-Host 'Start the waiver kiosk automatically when this Windows account signs in? (Y/N)'
}
